...
%     plot([community_area(:).areas_shape_ECEF])
    hold on
...
%         plot([community_area(i).areas_x_vector{1}],[community_area(i).areas_y_vector{1}],'k','Linewidth',0.8)
        ...
        if ~isempty(community_area(i).UAMstops_x) && ~isempty(UAMstops)
            p_UAMstops = plot(community_area(i).UAMstops_x(1),community_area(i).UAMstops_y(1),'ko','MarkerSize',3,'LineWidth',1);
